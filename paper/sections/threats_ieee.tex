...
non-disclosure agreement and cannot be released in its raw form. We provide
anonymized pre-computed SBERT embeddings and structural features, which are
sufficient to reproduce the training and evaluation pipeline but do not
allow qualitative inspection of the original test descriptions or commit
messages. We acknowledge that this limits the community's ability to
audit the semantic stream's behavior on the industrial data. Importantly,
...
and anonymized feature values. Additionally, the replication package
includes the complete distribution of per-build APFD values, structural
feature statistics (means, variances, correlations), and graph
connectivity metrics (node degree distributions, edge type proportions),
enabling the community to characterize the dataset's properties without
...
any access restrictions. The anonymized industrial replication package
preserves temporal ordering, pass/fail sequences, co-occurrence patterns,
and pre-computed embeddings, enabling reproduction of all reported metrics
even though the original text is not available.
