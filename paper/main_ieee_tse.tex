...
\author{
  \IEEEauthorblockN{Acauan C. Ribeiro\IEEEauthorrefmark{1},
    ...
    Jose C. R. do Nascimento\IEEEauthorrefmark{2},
    and N\'{i}colas R. G. Assump\c{c}\~{a}o\IEEEauthorrefmark{2}}
...
structural representation per test case (see Table~S7 in the supplementary
material for the complete configuration). The sensitivity analysis (Section~\ref{sec:rq4})
...
patience 7) per project. Table~S6 in the supplementary material provides the
complete training configuration for each setting.
...
For our experiments, we derive an anonymized version of this dataset that
preserves the properties essential for modeling: (1)~temporal ordering of
builds is maintained (build timestamps are replaced with monotonically
increasing indices); (2)~test case identities are replaced with opaque
identifiers while preserving the pass/fail outcome sequence and co-occurrence
patterns; (3)~semantic text (test descriptions, commit messages) is replaced
with Sentence-BERT embedding vectors, preserving the semantic similarity
structure without exposing proprietary content; and (4)~all structural
features (failure rate, recency, execution time) are computed from the
anonymized records. Owing to the non-disclosure agreement governing this
dataset, this anonymized version is used only for our internal evaluation and
is not part of the public replication package.
...
Table~S6 in the supplementary material lists the training
...
% [Table moved to supplementary material -- see Table S6]

% [Table moved to supplementary material -- see Table S7]
...
\textbf{Data Availability.} Our replication package, including the source
code for \filopriori{} and all baselines, trained model weights,
configuration files, and scripts to reproduce the RTPTorrent experiments
end-to-end, is publicly available at:
\url{https://github.com/filotransformer/filopriori}. The industrial QTA
dataset is proprietary and governed by a non-disclosure agreement; it cannot
be redistributed, and the industrial experiments are therefore not externally
reproducible.
...
\textbf{Conflicts of Interest.} Authors Jose C. R. do Nascimento and
N\'{i}colas R. G. Assump\c{c}\~{a}o are employed by Motorola Mobility
Com\'{e}rcio de Produtos Eletr\^{o}nicos Ltda., which provided the
industrial dataset analyzed in this study and supported the research
under the Brazilian Federal Law No.\ 8387/1991. The remaining authors
declare no competing interests. Motorola Mobility had no role in study
design, data analysis, interpretation of results, or the decision to
...
\begin{IEEEbiographynophoto}{Jose Carlos Rangel do Nascimento}
...
\begin{IEEEbiographynophoto}{N\'{i}colas Riccieri Gardin Assump\c{c}\~{a}o}
is a Staff Engineer at Motorola Mobility Com\'{e}rcio de Produtos Eletr\^{o}nicos Ltda.,
where he coordinates quality assurance projects focused on artificial intelligence.
He holds a Master's degree in Computer Science and a Bachelor's degree in
Computer Engineering, both from the State University of Campinas (UNICAMP),
Brazil. Previously, he worked as a software developer at the Instituto de
Pesquisas Eldorado, where he specialized in test automation and software tools
for a global project. His professional interests include test automation,
artificial intelligence applied to software quality, and continuous integration
