...
\begin{abstract}
In Continuous Integration (CI) environments, growing test suites make exhaustive
testing impractical, motivating Test Case Prioritization (TCP) to maximize early
fault detection. Existing TCP approaches typically deal with test cases as independent
entities, ignoring the structural relationships inherent in real-world test
suites: tests that co-fail share underlying dependencies, and temporal execution
patterns reveal systematic connections. In this paper, we present \filopriori{}, a graph-based
deep learning framework for TCP that models inter-test relationships through
Graph Attention Networks (GATv2) operating over a co-failure test relationship
graph, complemented by a Deep Neural Network (DNN)
ensemble with data-driven blending for semantically-sparse settings. We
compare \filopriori{} with five baselines on two complementary datasets: an
industrial CI pipeline (52,102 test executions from 2,347 unique test cases
across 1,339 builds) and the RTPTorrent benchmark (20 open-source Java
projects totaling over 100,000 builds with 2,937 containing failures). On
the industrial dataset, \filopriori{} achieves an APFD of 0.761 with
statistically significant improvements over all five baselines ($+10.2\%$ to
$+27.6\%$, $p < 0.001$). On
RTPTorrent, \filopriori{} achieves the highest numerical APFD of 0.854. However,
after Holm-Bonferroni correction only the improvements over two baselines
remain statistically robust, while margins over the top three
baselines represent a statistical tie, indicating that the framework's
substantial advantage requires sufficient co-failure graph density. Moreover, ablation
studies reveal that co-failure edges alone are the decisive graph component
($+17.0\%$ in isolation, $p < 0.001$); supplementary edge types do not
improve aggregate performance. The evaluation yields two notable
results: (1)~generic semantic embeddings (Sentence-BERT) provide no
statistically significant improvement ($p = 0.309$), confirmed by a
Random-Fixed control on both a baseline and \filopriori{} itself
($p = 0.965$); and (2)~\filopriori{}'s dual-balancing strategy is not
transferable to flat-feature architectures, confirming that the gain is
structural, not methodological. In addition, temporal cross-validation and sensitivity
analysis confirm robustness across time periods and hyperparameter choices.
\end{abstract}
...
\url{https://github.com/[anonymized]/filo-priori}
