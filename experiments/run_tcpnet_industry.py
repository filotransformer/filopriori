#!/usr/bin/env python3
"""
TCP-Net Experiment for Industry Dataset

Runs TCP-Net (Abdelkarim et al., ICSOFT 2022) on the 01_industry dataset.
Results are scientifically comparable with Filo-Priori, DeepOrder, and RETECS.
...
Usage:
    python experiments/run_tcpnet_industry.py
"""
...
CONFIG = {
    ...
    'duration_col': None,
    'seed': 42,
    'output_dir': 'results/tcpnet_industry',
    'method_name': 'TCP-Net',
...
def run_experiment(config: Dict) -> Dict:
    ...
    # Initialize TCP-Net
    logger.info("Initializing TCP-Net model...")
    ...
    logger.info(f"Training TCP-Net on device: {device}")
    ...
    for build_id in test_builds_with_failures:
        ...
        # Update with incremental retraining
        prioritizer.update_and_maybe_retrain(test_ids, verdicts)
...
def save_results(results: Dict, config: Dict):
    ...
    with open(comparison_path, 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("TCP-Net - Industry Dataset Results\n")
        f.write("=" * 70 + "\n\n")
...
def print_results(results: Dict):
    summary = results['summary']
    print("\n" + "=" * 70)
    print("TCP-Net - Industry Dataset Results")
    print("=" * 70)
...
def main():
    print("\n" + "=" * 70)
    print("TCP-Net Experiment - Industry Dataset")
