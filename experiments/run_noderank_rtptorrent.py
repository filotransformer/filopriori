#!/usr/bin/env python3
"""
NodeRank Experiment for RTPTorrent Dataset

Runs NodeRank (Li et al., IEEE TSE 2024) on the 02_rtptorrent dataset,
iterating over all projects in MSR2/.

NodeRank uses mutation-based ensemble learning:
1. Graph Structure Mutation (GSM)
2. Node Feature Mutation (NFM)
3. GNN Model Mutation (GMM)
4. Ensemble: LR, RF, XGBoost, LightGBM, GB
...
CONFIG = {
    ...
    'seed': 42,
    'output_dir': 'results/noderank_rtptorrent',
...
def run_project(project_dir: Path, config: Dict) -> Optional[Dict]:
    ...
    # Prepare training DataFrame in the format NodeRankModel expects
...
def _save_results(output_dir, all_project_results, all_build_results, total_start):
    ...
    with open(output_dir / 'comparison_summary.txt', 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("NodeRank - RTPTorrent Dataset Results\n")
        f.write("=" * 70 + "\n\n")
...
def main():
    print("\n" + "=" * 70)
    print("NodeRank Experiment - RTPTorrent Dataset")
    print("=" * 70 + "\n")
    ...
    # Print results
    print("\n" + "=" * 70)
    print("NodeRank - RTPTorrent Results Summary")
    print("=" * 70)
