#!/usr/bin/env python3
"""
FailRank-BB Experiment for RTPTorrent Dataset

Runs FailRank-BB (Hernandes et al., 2024) on the 02_rtptorrent dataset,
...
CONFIG = {
    ...
    'seed': 42,
    'output_dir': 'results/failrank_bb_rtptorrent',
...
def run_project(project_dir: Path, config: Dict) -> Optional[Dict]:
    ...
    # Prepare training DataFrame in the format FailRankBBModel expects
    ...
    # Initialize FailRank-BB
    # For RTPTorrent: use test names as TC text, no commit data available
    ...
    # Training phase
    # tc_text_col='TC_Key' encodes test names with SBERT
    # commit_text_col=None means commit component is zero vector
    ...
    # Pre-encode any test names not seen during training
    all_test_names = test_df['testName'].unique().tolist()
    new_names = [n for n in all_test_names if n not in model._tc_embedding_cache]
    if new_names:
        logger.info(f"  Encoding {len(new_names)} new test names from test set...")
        model._load_sbert()
        model._encode_texts(new_names, model._tc_embedding_cache)

    # Free SBERT model after all encoding is done
    model._unload_sbert()
    ...
    for i, build_id in enumerate(test_builds):
        ...
        # Prioritize using cached test name embeddings (no commit component)
...
def _save_results(output_dir, all_project_results, all_build_results, total_start):
    ...
    with open(output_dir / 'comparison_summary.txt', 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("FailRank-BB - RTPTorrent Dataset Results\n")
        f.write("=" * 70 + "\n\n")
...
def main():
    print("\n" + "=" * 70)
    print("FailRank-BB Experiment - RTPTorrent Dataset")
    print("=" * 70 + "\n")
    ...
    # Print results
    print("\n" + "=" * 70)
    print("FailRank-BB - RTPTorrent Results Summary")
    print("=" * 70)
