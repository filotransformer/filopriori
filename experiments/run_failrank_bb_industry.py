#!/usr/bin/env python3
"""
FailRank-BB Experiment for Industry Dataset

Runs FailRank-BB (Hernandes et al., 2024) on the 01_industry dataset
with scientific comparability to Filo-Priori.

Guarantees:
1. Uses EXACTLY the same train/test split (train.csv / test.csv)
...
Usage:
    python experiments/run_failrank_bb_industry.py
"""
...
from src.baselines.failrank_bb import FailRankBBModel
...
CONFIG = {
    ...
    'tc_text_col': 'TC_Steps',
    'commit_text_col': 'commit',

    # FailRank-BB hyperparameters
    'sbert_model_name': 'google-bert/bert-base-uncased',
    'max_iter': 1000,
    ...
    # Output
    'output_dir': 'results/failrank_bb_industry',
    'method_name': 'FailRank-BB',
...
def normalize_results(df: pd.DataFrame, config: Dict) -> pd.DataFrame:
    ...
    Filo-Priori rule that only 'Fail' is considered a failure.
    """
    df = df.copy()
...
def run_experiment(config: Dict) -> Dict:
    """
    Run FailRank-BB experiment with scientific comparability.
    ...
    tc_text_col = config['tc_text_col']
    commit_text_col = config['commit_text_col']

    # Initialize FailRank-BB model
    logger.info("Initializing FailRank-BB model...")
    model = FailRankBBModel(
        sbert_model_name=config['sbert_model_name'],
        max_iter=config['max_iter'],
        use_historical_features=config['use_historical_features'],
        seed=config['seed'],
        batch_size=config['batch_size'],
    )

    # Train on training data
    logger.info("Training FailRank-BB model...")
    ...
    model.train(
        ...
        tc_text_col=tc_text_col,
        commit_text_col=commit_text_col,
    )
    train_time = time.time() - train_start
    logger.info(f"Training completed in {train_time:.2f}s")

    # Pre-encode test TC_Steps texts not seen during training
    logger.info("Pre-encoding test TC texts...")
    if tc_text_col and tc_text_col in test_df.columns:
        test_tc_texts = test_df[tc_text_col].fillna('').astype(str).unique().tolist()
        new_texts = [t for t in test_tc_texts if t not in model._tc_embedding_cache]
        if new_texts:
            logger.info(f"  Encoding {len(new_texts)} new TC texts from test set...")
            model._load_sbert()
            model._encode_texts(new_texts, model._tc_embedding_cache)
            model._unload_sbert()
        logger.info(f"  Total cached TC embeddings: {len(model._tc_embedding_cache)}")

    # Pre-compute commit embeddings per test build
    logger.info("Pre-computing test build commit embeddings...")
    test_commit_embeddings = {}
    if commit_text_col and commit_text_col in test_df.columns:
        new_commits = set()
        build_commits_map = {}
        for build_id, group in test_df.groupby(build_col):
            commits = set()
            for val in group[commit_text_col].dropna():
                parsed = model._parse_commit_column(val)
                commits.update(parsed)
                new_commits.update(parsed)
            build_commits_map[build_id] = list(commits)

        # Encode any new commit texts
        new_commits_list = [c for c in new_commits if c not in model._commit_embedding_cache]
        if new_commits_list:
            logger.info(f"  Encoding {len(new_commits_list)} new commits from test set...")
            model._load_sbert()
            model._encode_texts(new_commits_list, model._commit_embedding_cache)
            model._unload_sbert()

        # Compute mean vectors
        for build_id, commits in build_commits_map.items():
            if commits:
                vecs = np.array([model._commit_embedding_cache[c] for c in commits],
                                dtype=np.float32)
                test_commit_embeddings[build_id] = vecs.mean(axis=0)
    ...
    for build_id in test_builds_with_failures:
        ...
        # Get TC texts for this build
        tc_texts = None
        if tc_text_col and tc_text_col in build_df.columns:
            # Build a mapping of TC_Key -> TC text
            tc_text_map = {}
            for _, row in build_df.iterrows():
                tc_text_map[row[test_col]] = str(row[tc_text_col]) if pd.notna(row[tc_text_col]) else ''
            tc_texts = [tc_text_map.get(tc, '') for tc in test_ids]

        # Get commit embedding for this build
        build_commit_emb = test_commit_embeddings.get(build_id, None)

        # Get prioritization ranking from FailRank-BB
        ranking = model.prioritize(
            test_ids,
            tc_texts=tc_texts,
            build_commit_embedding=build_commit_emb,
        )
        ...
        if apfd is not None:
            ...
            # Count commits (match Filo-Priori format)
            count_commits = 0
...
def save_results(results: Dict, config: Dict):
    ...
    with open(comparison_path, 'w') as f:
        f.write("="*70 + "\n")
        f.write("FailRank-BB - Industry Dataset Results\n")
        f.write("="*70 + "\n\n")
...
def print_results(results: Dict):
    ...
    print("\n" + "="*70)
    print("FailRank-BB - Industry Dataset Results")
    print("="*70)
...
def main():
    """Main entry point."""
    print("\n" + "="*70)
    print("FailRank-BB Experiment - Industry Dataset")
    ...
    print(f"  - FailRank-BB: {CONFIG['output_dir']}/apfd_per_build_FULL_testcsv.csv")
