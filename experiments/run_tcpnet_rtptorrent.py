#!/usr/bin/env python3
"""
TCP-Net Experiment for RTPTorrent Dataset

Runs TCP-Net (Abdelkarim et al., ICSOFT 2022) on the 02_rtptorrent dataset,
...
from src.baselines.tcpnet import TCPNetPrioritizer
...
CONFIG = {
    ...
    'seed': 42,
    'output_dir': 'results/tcpnet_rtptorrent',
    'method_name': 'TCP-Net',
    'train_ratio': 0.8,
    'device': DEFAULT_DEVICE,
    'epochs': 10,  # Reduced from 30 - RTPTorrent projects have large training sets
    'batch_size': 128,  # Increased from 64 - faster training
    'learning_rate': 0.001,
    'dropout': 0.3,
    'early_stopping_patience': 5,  # Reduced from 10 - faster early stopping
    'retrain_interval': 200,  # Increased from 50 - fewer retrains during evaluation
}
...
def run_project(project_dir: Path, config: Dict) -> Optional[Dict]:
    ...
    # Prepare training DataFrame in the format TCPNetPrioritizer expects
    train_df = df[df['travisJobId'].isin(train_builds)].copy()
    train_df['Build_ID'] = train_df['travisJobId']
    train_df['TC_Key'] = train_df['testName']
    # Convert is_failure to 'Fail'/'Pass' for compatibility with TCPNetPrioritizer.train()
    ...
    for build_id in test_builds:
        build_df = df[df['travisJobId'] == build_id]
        test_ids = build_df['testName'].unique().tolist()

        verdicts = {}
        durations = {}
        for _, row in build_df.iterrows():
            tc = row['testName']
            verdict = int(row['is_failure'])
            verdicts[tc] = max(verdicts.get(tc, 0), verdict)
            durations[tc] = float(row['duration'])

        n_failures = sum(verdicts.values())

        if n_failures == 0:
            # Still update history for temporal consistency
...
def main():
    print("\n" + "=" * 70)
    print("TCP-Net Experiment - RTPTorrent Dataset")
    print("=" * 70 + "\n")
    ...
    with open(output_dir / 'comparison_summary.txt', 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("TCP-Net - RTPTorrent Dataset Results\n")
        f.write("=" * 70 + "\n\n")
    ...
    # Print results
    print("\n" + "=" * 70)
    print("TCP-Net - RTPTorrent Results Summary")
    print("=" * 70)
    print(f"\nProjects: {aggregate['n_projects']}")
    print(f"Grand Mean APFD: {aggregate['grand_mean_apfd']:.4f} <<< PRIMARY METRIC")
    print(f"Grand Std APFD:  {aggregate['grand_std_apfd']:.4f}")
    print(f"\nPer-project:")
    for r in all_project_results:
        print(f"  {r['project']:40s} APFD={r['mean_apfd']:.4f} (n={r['n_builds_with_failures']})")
    print(f"\nTotal time: {total_time:.2f}s")
